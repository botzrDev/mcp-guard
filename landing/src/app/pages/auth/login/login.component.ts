                ...
                <div class="login-card">
                    ...
                    @if (authService.magicLinkSent()) {
                        <div class="magic-link-success">
                            ...
                            <p>We sent a magic link to <strong>{{ authService.magicLinkEmail() }}</strong></p>
                            <p class="hint">Click the link in the email to sign in. The link expires in 15 minutes.</p>
                            <button class="back-btn" (click)="resetMagicLink()">Use a different email</button>
                        </div>
                    } @else {
...
export class LoginComponent implements OnInit {
    ...
    sendMagicLink(): void {
        ...
        if (emailValue) {
            this.authService.sendMagicLink(emailValue);
        }
    }

    resetMagicLink(): void {
        this.authService.resetMagicLinkState();
        this.email.set('');
    }
}
